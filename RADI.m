...
for j=2:z_length
    delta_phi(1,j)= phi(j)-phi(j-1);                        %depth dependent porosity loss
    delta_phiS(1,j)= phiS(j)-phiS(j-1);                %depth dependent solid fraction gain
    delta_tort2(1,j)=tort(j)^2-tort(j-1)^2;               %depth dependent tortuosity gain
    delta_D_bio(1,j)=D_bio(j)-D_bio(j-1);          %[m/a]
end
...
for i=i:t_length-1
   ...
   % Aragonite dissolution rate from Dong et al. (2019) EPSL
       for j=1:z_length
          if OmegaA(1,j)<=1 && OmegaA(1,j)>0.835 %this is the omega value for which both laws are equals
          Rd_aragonite(1,j)=(Aragonite(1,j).*100.09*4*(0.0019*(1-OmegaA(1,j)).^0.13))/25;
          elseif OmegaA(1,j)<=0.835 %this is the omega value for which both laws are equals
          Rd_aragonite(1,j)=(Aragonite(1,j).*100.09*4*(0.021*(1-OmegaA(1,j)).^1.46))/25;    
          else            
          Rd_aragonite(1,j)=0;    
          end
       end
    ...
    for j=1:z_length
          if OmegaC(1,j)<=1 && OmegaC(1,j)>0.8275 %this is the omega value for which both laws are equals
          Rd_calcite(1,j)=(Calcite(1,j).*100.09*4*(0.00158*(1-OmegaC(1,j)).^0.11))/25;
          elseif OmegaC(1,j)<=0.8275 %this is the omega value for which both laws are equals
          Rd_calcite(1,j)=(Calcite(1,j).*100.09*4*(5*(1-OmegaC(1,j)).^4.7))/25;    
          else            
          Rd_calcite(1,j)=0;    
          end
    end
    ...
    for j=1:z_length
         if Calcite(1,j)<23500
            if OmegaC(1,j)>1
%            Rp_calcite(1,j)=(Calcite(1,j).*100.09*4*1.63*(OmegaC(1,j)-1).^1.76)/100;
            Rp_calcite(1,j)=(1.63*(OmegaC(1,j)-1).^1.76);
            else 
            Rp_calcite(1,j)=0;
            end
         else
