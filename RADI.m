...
% Requires: CO2SYS and calc_pco2

rerun = 0;

disp("RADIv19 is running the following experiment:")
disp(Station)
if rerun==1
...
%% Carbonate system initialization
% This is used only at the first time step to initialize the calc_pco2
% programme.
CO2SYSr = CO2SYS(TAw*1e6/rho_sw, DICw*1e6/rho_sw, 1, 2, S, T, T, ...
    P*10, P*10, Siw*1e6/rho_sw, PO4w*1e6/rho_sw, 1, 10, 1);
k1(1, 1:z_length) = CO2SYSr(1, 67); % carbonic acid 1st dissoc constant
k2(1, 1:z_length) = CO2SYSr(1, 68); % carbonic acid 2nd dissoc constant
...
kw(1, 1:z_length) = CO2SYSr(1, 71); % water dissociation constant
ksi(1, 1:z_length) = CO2SYSr(1, 78); % silica constants
bt(1, 1:z_length) = CO2SYSr(1, 79)*1e-6; % [mol/kg] total boron 
omegaC = CO2SYSr(1, 30); % calcite saturation state
omegaA = CO2SYSr(1, 31); % aragonite saturation state
co3 = CO2SYSr(1, 22)*1e-6; % [mol/kg] CO3 
hco3 = CO2SYSr(1, 21)*1e-6; % [mol/kg] HCO3
...
sit = 120*1e-6; % [mol/kg] set silica concentration

%% Temperature-dependent "free solution" diffusion coefficients [m2/a]
D_TA   = 0.015169 + 0.000793*T; % approximated to bicarbonate diffusion coefficient from Hulse et al (2018)
D_DIC  = 0.015169 + 0.000793*T; % approximted to bicarbonate diffusion coefficient from Hulse et al (2018)
D_Ca   = 0.0107   + 0.001677*T; % calcium diffusion coefficient from Li and Gregory
D_O2   = 0.034862 + 0.001409*T; % oxygen diffusion coefficient from Li and Gregiry
D_NO3  = 0.030842 + 0.001226*T; % nitrate diffusion coefficient from Li and Gregory (1974)
D_SO4  = 0.015768 + 0.000788*T; % sulfate diffusion coefficient from Li and Gregory (1974)
D_PO4  = 0.011291 + 0.000559*T; % phosphate diffusion coefficient from Li and Gregory (1974)
D_NH4  = 0.030905 + 0.001226*T; % ammonium diffusion coefficient from Li and Gregory (1974)
D_H2S  = 0.030748 + 0.000964*T; % hydrogen sulfide diffusion coefficient from the UNISENSE table by Ramsing and Gundersen
D_Mn2p = 0.0086   + 0.001525*T; % manganese diffusion coefficient from Li and Gregory (1974)
D_Fe2p = 0.0108   + 0.001478*T; % iron diffusion coefficient from Li and Gregory (1974)

%% Bioturbation (for solids)
D_bio_0 = 1e-4*0.0232*(Foc*1e2)^0.85;                                              %[m2/a] surf bioturb coeff, Archer et al (2002)
D_bio = D_bio_0*exp(-(z/0.08).^2)*((O2w/1e-3)/(O2w/1e-3 + 20));
% [m2/a] bioturb coeff, Archer et al (2002)

%% Irrigation (for solutes)
alpha_0 = 11*(atan((5*Foc*1e2 - 400)/400)/3.1416 + 0.5) - 0.9 + ...
    20*((O2w/1e-3)/(O2w/1e-3 + 10))* ...
    exp(-(O2w/1e-3)/10)*Foc*1e2/(Foc*1e2+30);
% [/a] from Archer et al (2002)
alpha = alpha_0*exp(-(z/0.05).^2);
...
Peh = w.*z_res./(2*D_bio); % half the cell Peclet number (Eq. 97 in Boudreau 1996)
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
if rerun == 0 
% concentrations set to zero for solids, bottom water values for
% not-too-sensitive solutes
    TA(1, 1:z_length) = TAw;
...
for i = i:t_length-1
    ...
    % Convert units to mol/kg
    DIC_molPerKg = DIC./rho_sw;
    TA_molPerKg = TA./rho_sw;
    PO4_molPerKg = PO4./rho_sw;
    H = calc_pCO2(DIC_molPerKg, PO4_molPerKg, sit, bt, TA_molPerKg, ...
        ff, k1, k2, k1p, k2p, k3p, kb, kw, ksi, fg, H); %[mol/kg] H concentration
    co3_molPerKg = DIC_molPerKg.*k1.*k2./(H.^2 + k1.*H + k1.*k2); %[mol/kg] CO3 concentration
    co3 = co3_molPerKg.*rho_sw; %[mol m^-3] CO3 concentration
...
%     % Aragonite dissolution rate from Dong et al. (2018) GCA
%     Rd_aragonite = Aragonite*2.*(1 - OmegaA).^1.4;
    ...
    Rp_calcite2 = zeros(size(OmegaC));
    J = Calcite < 23500 & OmegaC > 1;
    if any(J)
%         Rp_calcite(J) = (Calcite(J)*100.09*4*1.63.* ...
%             (OmegaC(J) - 1).^1.76)/100;
        Rp_calcite2(J) = 1.63*(OmegaC(J) - 1).^1.76;
    end
    ...
    fMnO2 = MnO2./(MnO2 + KMnO2).*(KinNO3./(KinNO3 + NO3)).* ...
        (KinO2./(KinO2 + O2));
    fFe3 = FeOH3./(FeOH3 + KFeOH3).*(KinMnO2./(MnO2 + KinMnO2)).* ...
        (KinNO3./(KinNO3 + NO3)).*(KinO2./(KinO2 + O2));
    fSO4 = SO4./(SO4 + KSO4).*(KinFeOH3./(FeOH3 + KinFeOH3)).* ...
        (KinMnO2./(MnO2 + KinMnO2)).*(KinNO3./(KinNO3 + NO3)).* ...
        (KinO2./(KinO2 + O2));
    fCH4 = (KinSO4./(SO4 + KinSO4)).*(KinFeOH3./(FeOH3 + KinFeOH3)).* ...
        (KinMnO2./(MnO2 + KinMnO2)).*(KinNO3./(KinNO3 + NO3)).* ...
        (KinO2./(KinO2 + O2));
    ...
    TotR = [+ phiS./phi.*Rg.*((RN/RC - RP/RC).*fO2 + ...
            ...
            2.*Rfeox - 2.*Rmnox - 2.*Rsox - 2.*Rnhox; % TA
            
            + phiS./phi.*Rg.* ...
            (fO2 + fNO3 + fMnO2 + fFe3 + fSO4 + fCH4*0.5) + ...
            phiS./phi.*(Rd_calcite + Rd_aragonite - Rp_calcite); % DIC
            
            - Rd_calcite + Rp_calcite; % phi./phiS.*Rp_calcite; % Calcite
            
            - Rd_aragonite; % Aragonite
            
            - phiS./phi.*Rg.*fO2 - 2.*Rsox - 2.*Rnhox - 0.5.*Rmnox - ...
            0.25.*Rfeox; % O2
