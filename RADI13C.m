...
SiBeta = 33;   %porosity attenuation coefficient
SiInf = 310;   %porosity at infinite depth
Si0 = 150;    %porosity at interface
sit = ((Si0 - SiInf)*exp(-SiBeta*depths) + SiInf).* 10^-6;   %porosity profile 
...
%% organic matter degradation parameters
 ...
 KdtNO3=0.005; %[mol/m3] Monod constant from Soetaert et al. 1996 (GCA)
 KindtNO3=0.002; %[mol/m3] Monod inhibition constant from Soetaert et al. 1996 (GCA)
 KpMnO2=200; %[mol/m3] Monod constant from Van Cappellen and Wang 1996
...
if rerun == 0 %concentrations set to zero for solids, bottom water values for not-too-sensitive solutes
    ...
    dt13CO2(1,1:ndepths)=dt13CO2w;            
    dt12CO2(1,1:ndepths)=dt12CO2w;            
    ...
    pro13c(1,1:ndepths)=fraction13roc*6e2;
    pro12c(1,1:ndepths)=fraction12roc*6e2;    
    psoc(1,1:ndepths)=3e2;             
    pso13c(1,1:ndepths)=fraction13soc*3e2;             
    pso12c(1,1:ndepths)=fraction12soc*3e2; 
    pfoc(1,1:ndepths)=1e1;        
    pfo13c(1,1:ndepths)=fraction13foc*1e1;        
    pfo12c(1,1:ndepths)=fraction12foc*1e1;        
    pFeOH3(1,1:ndepths)=0;    
    pMnO2(1,1:ndepths)=0;    
    pcalcite(1,1:ndepths)=4.5e3;
    p13calcite(1,1:ndepths)=fraction13calcite*4.5e3;
    p12calcite(1,1:ndepths)=fraction12calcite*4.5e3;
...
elseif rerun==1 %if it is a rerun, initial conditions are concentrations from last time step
    ...
    dt13CO2=dt13CO2f(:,idx-1)';      %[mol/m3]
    dt12CO2=dt12CO2f(:,idx-1)';      %[mol/m3]
    ...
    pro13c=pro13cf(:,idx-1)';                %[mol/m3]
    pro12c=pro12cf(:,idx-1)';                %[mol/m3]
    psoc=psocf(:,idx-1)';                %[mol/m3]
    pso13c=pso13cf(:,idx-1)';                %[mol/m3]
    pso12c=pso12cf(:,idx-1)';                %[mol/m3]
    pfoc=pfocf(:,idx-1)';                %[mol/m3]
    pfo13c=pfo13cf(:,idx-1)';                %[mol/m3]
    pfo12c=pfo12cf(:,idx-1)';                %[mol/m3]
    pFeOH3=pFeOH3f(:,idx-1)';                %[mol/m3]
    pMnO2=pMnO2f(:,idx-1)';                %[mol/m3]
    pcalcite=pcalcitef(:,idx-1)';            %[mol/m3]
    p13calcite=p13calcitef(:,idx-1)';            %[mol/m3]
    p12calcite=p12calcitef(:,idx-1)';            %[mol/m3]
    ...
    dt13CO2=dt13CO2ic';                %[mol/m3]
    dt12CO2=dt12CO2ic';                %[mol/m3]
    ...
    pMnO2=pMnO2ic';
    pcalcite=pcalciteic';
    p13calcite=p13calciteic';
    p12calcite=p12calciteic';
...
dt13CO2f = NaN(ndepths, stoptime+1);
dt12CO2f = NaN(ndepths, stoptime+1);
...
pro13cf = NaN(ndepths, stoptime+1);
pro12cf = NaN(ndepths, stoptime+1);
psocf = NaN(ndepths, stoptime+1);
pso13cf = NaN(ndepths, stoptime+1);
pso12cf = NaN(ndepths, stoptime+1);
pfocf = NaN(ndepths, stoptime+1);
pfo13cf = NaN(ndepths, stoptime+1);
pfo12cf = NaN(ndepths, stoptime+1);
pFeOH3f = NaN(ndepths, stoptime+1);
MnO2f = NaN(ndepths, stoptime+1);
pcalcitef = NaN(ndepths, stoptime+1);
p13calcitef = NaN(ndepths, stoptime+1);
p12calcitef = NaN(ndepths, stoptime+1);
...
for i=i:t_length-1
    ...
    %% Compute new 13C/12C ratios    
    fraction13tCO2=dt13CO2./(dt12CO2+dt13CO2);
    fraction12tCO2=dt12CO2./(dt12CO2+dt13CO2);
    fraction13foc=pfo13c./(pfo12c+pfo13c);
    fraction12foc=pfo12c./(pfo12c+pfo13c);
    fraction13soc=pso13c./(pso12c+pso13c);
    fraction12soc=pso12c./(pso12c+pso13c);
    fraction13roc=pro13c./(pro12c+pro13c);
    fraction12roc=pro12c./(pro12c+pro13c);
    fraction13calcite=p13calcite./(p12calcite+p13calcite);
    fraction12calcite=p12calcite./(p12calcite+p13calcite);

if sum(isnan(fraction13foc))>0
    break
end
   ...
   % Aragonite dissolution rate from Dong et al. (2019) EPSL
    ...
    TotR_dt13CO2 = phiS./phi.*(fraction13soc.*Rs_o2 + fraction13foc.*Rf_o2 + fraction13soc.*Rs_no3 + fraction13foc.*Rf_no3 ...
        + fraction13soc.*Rs_mno2 + fraction13foc.*Rf_mno2 + fraction13soc.*Rs_feoh3 + fraction13foc.*Rf_feoh3...
        + fraction13soc.*Rs_so4 + fraction13foc.*Rf_so4 + fraction13soc.*Rs_ch4.*0.5 + fraction13foc.*Rf_ch4.*0.5 + ...
        (fraction13calcite.*Rd_calcite + Rd_aragonite - fraction13tCO2.*Rp_calcite)); %note: no 13 or 12C in aragonite for now, add it in the future? 

    TotR_dt12CO2 = phiS./phi.*(fraction12soc.*Rs_o2 + fraction12foc.*Rf_o2 + fraction12soc.*Rs_no3 + fraction12foc.*Rf_no3 ...
        + fraction12soc.*Rs_mno2 + fraction12foc.*Rf_mno2 + fraction12soc.*Rs_feoh3 + fraction12foc.*Rf_feoh3...
        + fraction12soc.*Rs_so4 + fraction12foc.*Rf_so4 + fraction12soc.*Rs_ch4.*0.5 + fraction12foc.*Rf_ch4.*0.5 + ...
        (fraction12calcite.*Rd_calcite + Rd_aragonite - fraction12tCO2.*Rp_calcite)); %note: no 12 or 12C in aragonite for now, add it in the future? 
    ...
    TotR_pso13c = - fraction13soc.*Rs_tot;
    
    TotR_pso12c = - fraction12soc.*Rs_tot;    
    
    TotR_pfoc = - Rf_tot;

    TotR_pfo13c = - fraction13foc.*Rf_tot;
    
    TotR_pfo12c = - fraction12foc.*Rf_tot;    
    ...
    TotR_p13calcite = -fraction13calcite.*Rd_calcite + fraction13tCO2.*Rp_calcite; 

    TotR_p12calcite = -fraction12calcite.*Rd_calcite + fraction12tCO2.*Rp_calcite; 
    ...
    dt13CO2_1 = dt13CO2(1) + interval * ( D_dtCO2 / tort2(1) * (2*dt13CO2(2) - 2*dt13CO2(1) + TR(1) * (dt13CO2w - dt13CO2(1))) / (z_res(1)^2) ... %diffusion
        - (u(1) - D_dtCO2.*DFF(1)) * -1 * TR(1) * ( dt13CO2w - dt13CO2(1)) / (2*z_res(1)) ... %advection
        + alpha(1) * ( dt13CO2w - dt13CO2(1) ) ... %irrigation
        + TotR_dt13CO2(1)); %reaction

    dt12CO2_1 = dt12CO2(1) + interval * ( D_dtCO2 / tort2(1) * (2*dt12CO2(2) - 2*dt12CO2(1) + TR(1) * (dt12CO2w - dt12CO2(1))) / (z_res(1)^2) ... %diffusion
        - (u(1) - D_dtCO2.*DFF(1)) * -1 * TR(1) * ( dt12CO2w - dt12CO2(1)) / (2*z_res(1)) ... %advection
        + alpha(1) * ( dt12CO2w - dt12CO2(1) ) ... %irrigation
        + TotR_dt12CO2(1)); %reaction
    ...
    pro13c_1 = pro13c(1) + interval * ( D_bio(1) * ( 2 * pro13c(2) - 2 * pro13c(1) +... %diffusion
        2 * z_res(1) * (Fro13c - phiS(1) * w(1) * pro13c(1)) / (D_bio(1) * phiS(1)) ) / (z_res(1).^2) ...  %diffusion
        - APPW(1) * ((1 - sigma(1))*pro13c(2) + 2*sigma(1)*pro13c(1) - ... %advection
        (1 + sigma(1))*(pro13c(2)+2*z_res(1)/D_bio(1)*(Fro13c/phiS(1)-w(1)*pro13c(1))))/(2*z_res(1))); ... %advection

    pro12c_1 = pro12c(1) + interval * ( D_bio(1) * ( 2 * pro12c(2) - 2 * pro12c(1) +... %diffusion
        2 * z_res(1) * (Fro12c - phiS(1) * w(1) * pro12c(1)) / (D_bio(1) * phiS(1)) ) / (z_res(1).^2) ...  %diffusion
        - APPW(1) * ((1 - sigma(1))*pro12c(2) + 2*sigma(1)*pro12c(1) - ... %advection
        (1 + sigma(1))*(pro12c(2)+2*z_res(1)/D_bio(1)*(Fro12c/phiS(1)-w(1)*pro12c(1))))/(2*z_res(1))); ... %advection
    ...
    pso13c_1 = pso13c(1) + interval * ( D_bio(1) * ( 2 * pso13c(2) - 2 * pso13c(1) +... %diffusion
        2 * z_res(1) * (Fso13c - phiS(1) * w(1) * pso13c(1)) / (D_bio(1) * phiS(1)) ) / (z_res(1).^2) ...  %diffusion
        - APPW(1) * ((1 - sigma(1))*pso13c(2) + 2*sigma(1)*pso13c(1) - ... %advection
        (1 + sigma(1))*(pso13c(2)+2*z_res(1)/D_bio(1)*(Fso13c/phiS(1)-w(1)*pso13c(1))))/(2*z_res(1)) ... %advection
        +TotR_pso13c(1)); %reaction

    pso12c_1 = pso12c(1) + interval * ( D_bio(1) * ( 2 * pso12c(2) - 2 * pso12c(1) +... %diffusion
        2 * z_res(1) * (Fso12c - phiS(1) * w(1) * pso12c(1)) / (D_bio(1) * phiS(1)) ) / (z_res(1).^2) ...  %diffusion
        - APPW(1) * ((1 - sigma(1))*pso12c(2) + 2*sigma(1)*pso12c(1) - ... %advection
        (1 + sigma(1))*(pso12c(2)+2*z_res(1)/D_bio(1)*(Fso12c/phiS(1)-w(1)*pso12c(1))))/(2*z_res(1)) ... %advection
        +TotR_pso12c(1)); %reaction
    ...
    pfo13c_1 = pfo13c(1) + interval * ( D_bio(1) * ( 2 * pfo13c(2) - 2 * pfo13c(1) +... %diffusion
        2 * z_res(1) * (Ffo13c - phiS(1) * w(1) * pfo13c(1)) / (D_bio(1) * phiS(1)) ) / (z_res(1).^2) ...  %diffusion
        - APPW(1) * ((1 - sigma(1))*pfo13c(2) + 2*sigma(1)*pfo13c(1) - ... %advection
        (1 + sigma(1))*(pfo13c(2)+2*z_res(1)/D_bio(1)*(Ffo13c/phiS(1)-w(1)*pfo13c(1))))/(2*z_res(1)) ... %advection
        +TotR_pfo13c(1)); %reaction
    
    pfo12c_1 = pfo12c(1) + interval * ( D_bio(1) * ( 2 * pfo12c(2) - 2 * pfo12c(1) +... %diffusion
        2 * z_res(1) * (Ffo12c - phiS(1) * w(1) * pfo12c(1)) / (D_bio(1) * phiS(1)) ) / (z_res(1).^2) ...  %diffusion
        - APPW(1) * ((1 - sigma(1))*pfo12c(2) + 2*sigma(1)*pfo12c(1) - ... %advection
        (1 + sigma(1))*(pfo12c(2)+2*z_res(1)/D_bio(1)*(Ffo12c/phiS(1)-w(1)*pfo12c(1))))/(2*z_res(1)) ... %advection
        +TotR_pfo12c(1)); %reaction
    ...
    p13calcite_1 = p13calcite(1) + interval * ( D_bio(1) * ( 2 * p13calcite(2) - 2 * p13calcite(1) +... %diffusion
        2 * z_res(1) * (F13calcite - phiS(1) * w(1) * p13calcite(1)) / (D_bio(1) * phiS(1)) ) / (z_res(1).^2) ...  %diffusion
        - APPW(1) * ((1 - sigma(1))*p13calcite(2) + 2*sigma(1)*p13calcite(1) - ... %advection
        (1 + sigma(1))*(p13calcite(2)+2*z_res(1)/D_bio(1)*(F13calcite/phiS(1)-w(1)*p13calcite(1))))/(2*z_res(1)) ... %advection
        +TotR_p13calcite(1)); %reaction

    p12calcite_1 = p12calcite(1) + interval * ( D_bio(1) * ( 2 * p12calcite(2) - 2 * p12calcite(1) +... %diffusion
        2 * z_res(1) * (F12calcite - phiS(1) * w(1) * p12calcite(1)) / (D_bio(1) * phiS(1)) ) / (z_res(1).^2) ...  %diffusion
        - APPW(1) * ((1 - sigma(1))*p12calcite(2) + 2*sigma(1)*p12calcite(1) - ... %advection
        (1 + sigma(1))*(p12calcite(2)+2*z_res(1)/D_bio(1)*(F12calcite/phiS(1)-w(1)*p12calcite(1))))/(2*z_res(1)) ... %advection
        +TotR_p12calcite(1)); %reaction
    ...
    dt13CO2_z = dt13CO2(ndepths) + interval * (D_dtCO2 / tort2(ndepths) * 2 * ((dt13CO2(ndepths-1) - dt13CO2(ndepths)) / z_res(ndepths).^2) ...  %diffusion
        + alpha(ndepths) * (dt13CO2w - dt13CO2(ndepths)) ... %irrigation
        +TotR_dt13CO2(ndepths));

    dt12CO2_z = dt12CO2(ndepths) + interval * (D_dtCO2 / tort2(ndepths) * 2 * ((dt12CO2(ndepths-1) - dt12CO2(ndepths)) / z_res(ndepths).^2) ...  %diffusion
        + alpha(ndepths) * (dt12CO2w - dt12CO2(ndepths)) ... %irrigation
        +TotR_dt12CO2(ndepths));
    ...
    pro13c_z = pro13c(ndepths) + interval * (D_bio(ndepths) * 2 * ( (pro13c(ndepths-1) - pro13c(ndepths)) / z_res(ndepths).^2)... %diffusion
        - APPW(ndepths) * (-sigma(ndepths)*pro13c(ndepths-1) + sigma(ndepths)*pro13c(ndepths))/z_res(ndepths)); %advection

    pro12c_z = pro12c(ndepths) + interval * (D_bio(ndepths) * 2 * ( (pro12c(ndepths-1) - pro12c(ndepths)) / z_res(ndepths).^2)... %diffusion
        - APPW(ndepths) * (-sigma(ndepths)*pro12c(ndepths-1) + sigma(ndepths)*pro12c(ndepths))/z_res(ndepths)); %advection
    ...
    pso13c_z = pso13c(ndepths) + interval * (D_bio(ndepths) * 2 * ( (pso13c(ndepths-1) - pso13c(ndepths)) / z_res(ndepths).^2)... %diffusion
        - APPW(ndepths) * (-sigma(ndepths)*pso13c(ndepths-1) + sigma(ndepths)*pso13c(ndepths))/z_res(ndepths)... %advection
        +TotR_pso13c(ndepths));

    pso12c_z = pso12c(ndepths) + interval * (D_bio(ndepths) * 2 * ( (pso12c(ndepths-1) - pso12c(ndepths)) / z_res(ndepths).^2)... %diffusion
        - APPW(ndepths) * (-sigma(ndepths)*pso12c(ndepths-1) + sigma(ndepths)*pso12c(ndepths))/z_res(ndepths)... %advection
        +TotR_pso12c(ndepths));
    ...
    pfo13c_z = pfo13c(ndepths) + interval * (D_bio(ndepths) * 2 * ( (pfo13c(ndepths-1) - pfo13c(ndepths)) / z_res(ndepths).^2)... %diffusion
        - APPW(ndepths) * (-sigma(ndepths)*pfo13c(ndepths-1) + sigma(ndepths)*pfo13c(ndepths))/z_res(ndepths)... %advection
        +TotR_pfo13c(ndepths));

    pfo12c_z = pfo12c(ndepths) + interval * (D_bio(ndepths) * 2 * ( (pfo12c(ndepths-1) - pfo12c(ndepths)) / z_res(ndepths).^2)... %diffusion
        - APPW(ndepths) * (-sigma(ndepths)*pfo12c(ndepths-1) + sigma(ndepths)*pfo12c(ndepths))/z_res(ndepths)... %advection
        +TotR_pfo12c(ndepths));
    ...
    p13calcite_z = p13calcite(ndepths) + interval * (D_bio(ndepths) * 2 * ( (p13calcite(ndepths-1) - p13calcite(ndepths)) / z_res(ndepths).^2)... %diffusion
        - APPW(ndepths) * (-sigma(ndepths)*p13calcite(ndepths-1) + sigma(ndepths)*p13calcite(ndepths))/z_res(ndepths)... %advection
        +TotR_p13calcite(ndepths));

    p12calcite_z = p12calcite(ndepths) + interval * (D_bio(ndepths) * 2 * ( (p12calcite(ndepths-1) - p12calcite(ndepths)) / z_res(ndepths).^2)... %diffusion
        - APPW(ndepths) * (-sigma(ndepths)*p12calcite(ndepths-1) + sigma(ndepths)*p12calcite(ndepths))/z_res(ndepths)... %advection
        +TotR_p12calcite(ndepths));
    ...
    dt13CO2_j = dt13CO2(j);
    dt13CO2_jp1 = dt13CO2(jp1);
    dt13CO2_jm1 = dt13CO2(jm1);
    dt13CO2(j) = dt13CO2_j + interval*(TotR_dt13CO2(j) - ...
        (u_j - D_dtCO2*DFF_j).*(dt13CO2_jp1 - dt13CO2_jm1)./(2*z_res_j) + ...
        (D_dtCO2./tort2_j).*((dt13CO2_jp1 - 2*dt13CO2_j + dt13CO2_jm1)./z_res2_j) + ...
        alpha_j.*(dt13CO2w - dt13CO2_j));

    dt12CO2_j = dt12CO2(j);
    dt12CO2_jp1 = dt12CO2(jp1);
    dt12CO2_jm1 = dt12CO2(jm1);
    dt12CO2(j) = dt12CO2_j + interval*(TotR_dt12CO2(j) - ...
        (u_j - D_dtCO2*DFF_j).*(dt12CO2_jp1 - dt12CO2_jm1)./(2*z_res_j) + ...
        (D_dtCO2./tort2_j).*((dt12CO2_jp1 - 2*dt12CO2_j + dt12CO2_jm1)./z_res2_j) + ...
        alpha_j.*(dt12CO2w - dt12CO2_j));
   ...
   % Refractory organic carbon
    ...
    pro13c_j = pro13c(j);
    pro13c_jp1 = pro13c(jp1);
    pro13c_jm1 = pro13c(jm1);
    pro13c(j) = pro13c_j + interval*(... 
        - APPW_j.*(((1 - sigma_j).*pro13c_jp1 + ...
        2*sigma_j.*pro13c_j - ...
        (1 + sigma_j).*pro13c_jm1)./(2*z_res_j)) + ...
        D_bio_j.*((pro13c_jp1 - 2*pro13c_j + ...
        pro13c_jm1)./z_res2_j));

    pro12c_j = pro12c(j);
    pro12c_jp1 = pro12c(jp1);
    pro12c_jm1 = pro12c(jm1);
    pro12c(j) = pro12c_j + interval*(... 
        - APPW_j.*(((1 - sigma_j).*pro12c_jp1 + ...
        2*sigma_j.*pro12c_j - ...
        (1 + sigma_j).*pro12c_jm1)./(2*z_res_j)) + ...
        D_bio_j.*((pro12c_jp1 - 2*pro12c_j + ...
        pro12c_jm1)./z_res2_j));
    ...
    pso13c_j = pso13c(j);
    pso13c_jp1 = pso13c(jp1);
    pso13c_jm1 = pso13c(jm1);
    pso13c(j) = pso13c_j + interval*(TotR_pso13c(j) - ...
        APPW_j.*(((1 - sigma_j).*pso13c_jp1 + ...
        2*sigma_j.*pso13c_j - ...
        (1 + sigma_j).*pso13c_jm1)./(2*z_res_j)) + ...
        D_bio_j.*((pso13c_jp1 - 2*pso13c_j + ...
        pso13c_jm1)./z_res2_j));
    
    pso12c_j = pso12c(j);
    pso12c_jp1 = pso12c(jp1);
    pso12c_jm1 = pso12c(jm1);
    pso12c(j) = pso12c_j + interval*(TotR_pso12c(j) - ...
        APPW_j.*(((1 - sigma_j).*pso12c_jp1 + ...
        2*sigma_j.*pso12c_j - ...
        (1 + sigma_j).*pso12c_jm1)./(2*z_res_j)) + ...
        D_bio_j.*((pso12c_jp1 - 2*pso12c_j + ...
        pso12c_jm1)./z_res2_j));
    ...
    pfo13c_j = pfo13c(j);
    pfo13c_jp1 = pfo13c(jp1);
    pfo13c_jm1 = pfo13c(jm1);
    pfo13c(j) = pfo13c_j + interval*(TotR_pfo13c(j) - ...
        APPW_j.*(((1 - sigma_j).*pfo13c_jp1 + ...
        2*sigma_j.*pfo13c_j - ...
        (1 + sigma_j).*pfo13c_jm1)./(2*z_res_j)) + ...
        D_bio_j.*((pfo13c_jp1 - 2*pfo13c_j + ...
        pfo13c_jm1)./z_res2_j));
    
    pfo12c_j = pfo12c(j);
    pfo12c_jp1 = pfo12c(jp1);
    pfo12c_jm1 = pfo12c(jm1);
    pfo12c(j) = pfo12c_j + interval*(TotR_pfo12c(j) - ...
        APPW_j.*(((1 - sigma_j).*pfo12c_jp1 + ...
        2*sigma_j.*pfo12c_j - ...
        (1 + sigma_j).*pfo12c_jm1)./(2*z_res_j)) + ...
        D_bio_j.*((pfo12c_jp1 - 2*pfo12c_j + ...
        pfo12c_jm1)./z_res2_j));
    ...
    p13calcite_j = p13calcite(j);
    p13calcite_jp1 = p13calcite(jp1);
    p13calcite_jm1 = p13calcite(jm1);
    p13calcite(j) = p13calcite_j + interval*(TotR_p13calcite(j) - ...
        APPW_j.*(((1 - sigma_j).*p13calcite_jp1 + ...
        2*sigma_j.*p13calcite_j - ...
        (1 + sigma_j).*p13calcite_jm1)./(2*z_res_j)) + ...
        D_bio_j.*((p13calcite_jp1 - 2*p13calcite_j + ...
        p13calcite_jm1)./z_res2_j));    

    p12calcite_j = p12calcite(j);
    p12calcite_jp1 = p12calcite(jp1);
    p12calcite_jm1 = p12calcite(jm1);
    p12calcite(j) = p12calcite_j + interval*(TotR_p12calcite(j) - ...
        APPW_j.*(((1 - sigma_j).*p12calcite_jp1 + ...
        2*sigma_j.*p12calcite_j - ...
        (1 + sigma_j).*p12calcite_jm1)./(2*z_res_j)) + ...
        D_bio_j.*((p12calcite_jp1 - 2*p12calcite_j + ...
        p12calcite_jm1)./z_res2_j));    
    ...
    p13calcite(1) = p13calcite_1;
    p12calcite(1) = p12calcite_1;
    paragonite(1) = paragonite_1;
    pclay(1) = pclay_1;
    dO2(ndepths) = dO2_z;
    dtalk(ndepths) = dtalk_z; 
    dtCO2(ndepths) = dtCO2_z; 
    dt13CO2(ndepths) = dt13CO2_z; 
    dt12CO2(ndepths) = dt12CO2_z; 
    ...
    dMn(ndepths) = dMn_z;
    proc(ndepths) = proc_z;
    pro13c(ndepths) = pro13c_z;
    pro12c(ndepths) = pro12c_z;
    psoc(ndepths) = psoc_z;
    pso13c(ndepths) = pso13c_z;
    pso12c(ndepths) = pso12c_z;
    pfoc(ndepths) = pfoc_z; 
    pfo13c(ndepths) = pfo13c_z; 
    pfo12c(ndepths) = pfo12c_z; 
    pFeOH3(ndepths) = pFeOH3_z;
    pMnO2(ndepths) = pMnO2_z; 
    pcalcite(ndepths) = pcalcite_z;
    p13calcite(ndepths) = p13calcite_z;
    p12calcite(ndepths) = p12calcite_z;
    ...
    dCa(dCa<0)=0;
    proc(proc<0)=0;
    pro13c(pro13c<0)=1e-20;
    pro12c(pro12c<0)=1e-20;
    psoc(psoc<0)=0;
    pso13c(pso13c<0)=1e-20;
    pso12c(pso12c<0)=1e-20;
    pfoc(pfoc<0)=0;
    pfo13c(pfo13c<0)=1e-20;
    pfo12c(pfo12c<0)=1e-20;
    pFeOH3(pFeOH3<0)=0;
    pMnO2(pMnO2<0)=0;    
    pcalcite(pcalcite<0)=0;    
    p13calcite(p13calcite<0)=1e-20;    
    p12calcite(p12calcite<0)=1e-20;    
    ...
    %pclayf(:, i+1) = pclay; 
     ...
     if i == plot_number(idx)
       ...
       dtalkf(:, idx) = dtalk;
       dtCO2f(:, idx) = dtCO2; 
       dt13CO2f(:, idx) = dt13CO2; 
       dt12CO2f(:, idx) = dt12CO2; 
       dtNO3f(:, idx) = dtNO3;
       dtSO4f(:, idx) = dtSO4;
       dtPO4f(:, idx) = dtPO4;
       dtNH4f(:, idx) = dtNH4;
       ...
       p13calcitef(:, idx) = p13calcite; 
       p12calcitef(:, idx) = p12calcite; 
